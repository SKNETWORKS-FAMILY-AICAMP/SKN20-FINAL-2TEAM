...
def parse_xml_to_json_per_image(xml_file_path, locarno_code):
    ...
    Args:
        ...
        locarno_code: 로카르노 분류 코드 (예: "09-01")
...
if __name__ == "__main__":
    ...
    if input_folder.exists():
        ...
        print("=" * 50)
        results, count = convert_folder_per_image(str(input_folder), str(output_folder), "reject")
        success_count = len([r for r in results if r['status'] == 'success'])
        print(f"\napi_xml: {success_count}/{len(results)} XML 변환 완료 → {count}개 JSON 파일 생성\n")
        ...
        if failed_files:
            ...
            for failed in failed_files:
                print(f"파일명: {failed['xml_file']}")
                print(f"오류: {failed.get('message', 'Unknown error')}\n")
        
        # 3개 미만 저장 파일 확인
        incomplete_files = [r for r in results if r['status'] == 'success' and r['json_count'] < 3]
        if incomplete_files:
            print("=" * 50)
            print("⚠️  3개 미만 저장된 파일 목록")
            print("=" * 50)
            for incomplete in incomplete_files:
                print(f"파일명: {incomplete['xml_file']} → {incomplete['json_count']}/3개 저장\n")
