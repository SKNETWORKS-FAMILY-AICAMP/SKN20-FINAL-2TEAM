...
def photo_to_sketch(img: np.ndarray, target_thickness: float = 3.0) -> np.ndarray:
    ...
    Args:
        img: 입력 이미지 (BGR 또는 그레이스케일)
        target_thickness: 목표 선 굵기 (기본 3픽셀)
    
    Returns:
        스케치 이미지 (배경=255, 선=0, 완전 이진화)
    """
    ...
    # Canny 엣지 검출 (임계값 30~100)
    # 결과: 엣지=255(흰색), 배경=0(검정)
    edges = cv2.Canny(denoised, 30, 100)
    
    # 선 굵기 조정
    current_thickness = estimate_line_thickness(edges)  # 현재 선 굵기 측정
    
    if current_thickness > 0 and current_thickness < target_thickness:
        # 현재 선이 목표보다 얇으면 굵게 만들기
        ratio = target_thickness / current_thickness  # 굵기 비율
        kernel_size = max(3, min(7, int(ratio * 2) | 1))  # 커널 크기 (3~7, 홀수)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))  # 타원형 커널
        edges = cv2.dilate(edges, kernel, iterations=1)  # Dilation (선 굵게)
    
    # 최종 이진화: 배경=255(흰색), 선=0(검정)
    # edges는 엣지=255, 배경=0이므로 반전 필요
    sketch_final = 255 - edges  # 반전: 배경=255, 선=0
