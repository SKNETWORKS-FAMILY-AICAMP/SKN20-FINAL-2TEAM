...
def hybrid_retrieve(
    ...
    for did in dense_ids:
        ...
        if app_num not in best_by_app or hybrid > best_by_app[app_num]["hybrid_score"]:
            best_by_app[app_num] = entry

    results = sorted(best_by_app.values(), key=lambda x: x["hybrid_score"], reverse=True)
