...
원본: chat.py
변경:
  1. RAG 호출: rag.backend_adapter_gpt.analyze_product (GPT 버전)
  2. _call_vllm_fallback → _call_gpt_fallback (vLLM 시도 없이 OpenAI 직접 호출)
  3. model_used: "gpt-4o-mini"
"""
...
# RAG 모듈 경로 추가 (backend/app/routers/chat_gpt.py 기준 4단계 상위 = 프로젝트 루트)
...
async def send_chat_message(
    ...
    """
    프론트엔드 채팅 인터페이스용 메시지 API (GPT 버전)
    - FTO 분석: RAG 파이프라인(ChromaDB + OpenAI GPT) 호출 → analyses 테이블 저장
    - 폴백: GPT-4o-mini 직접 호출
    """
    ...
    # ── FTO 분석: RAG 파이프라인 (GPT 버전) ──────────────────
    if analysis_type == "fto" and message.strip():
        start_time = time.time()
        try:
            # ── 변경: GPT 버전 backend_adapter 사용 ──
            ...
            analysis_obj = Analysis(
                ...
                model_used="gpt-4o-mini",  # ── 변경: GPT 모델명 ──
            ...
            # RAG 실패 시 GPT 직접 호출 폴백
            response_message = _call_gpt_fallback(db, chat_id, current_user.id, chat_service, e)

    else:
        # design 분석 또는 analysis_type이 fto가 아닌 경우: GPT 직접 호출
        response_message = _call_gpt_fallback(db, chat_id, current_user.id, chat_service, None)
...
# ── 변경: vLLM 시도 없이 OpenAI GPT 직접 호출 ──
def _call_gpt_fallback(db, chat_id: int, user_id: int, chat_service: "ChatService", err) -> str:
    """RAG 실패 또는 design 분석 시 GPT-4o-mini 직접 호출."""
    if err:
        print(f"[GPT 폴백] RAG 파이프라인 오류: {err}")
    ...
    # OpenAI GPT-4o-mini 직접 호출 (vLLM 시도 없음)
    openai_key = os.getenv("OPENAI_API_KEY", "")
    if not openai_key:
        return "OPENAI_API_KEY가 설정되지 않았습니다. .env 파일을 확인하세요."
    ...
    try:
        ...
        return f"GPT 호출 중 오류가 발생했습니다: {str(gpt_err)}"
