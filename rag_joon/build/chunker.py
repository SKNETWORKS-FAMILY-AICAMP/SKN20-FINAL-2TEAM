...
관계:
    ...
    - eval/build_index.py가 load_chunks_from_dir()를 호출
"""
import json
import re
...
def classify_claim(claim: dict) -> tuple[str, list[int]]:
    ...
    # JSON에 claim_type이 명시되어 있으면 그대로 사용
    if claim.get("claim_type"):
        return claim["claim_type"], claim.get("refers_to", [])

    # claim_type 없으면 "제N항"을 참조하는지 텍스트 패턴으로 판별
    text = claim.get("text", "")
...
def _safe_text(claim: dict) -> str | None:
    """삭제/빈/이미지 청구항 필터링. 유효한 텍스트만 반환, 나머지 None."""
    # 삭제된 청구항은 제외 (change_code="D" 또는 change_type="삭제")
    if claim.get("change_code") == "D" or claim.get("change_type") == "삭제":
        return None
    text = (claim.get("text") or "").strip()
    # 빈 텍스트이거나 이미지만 있는 청구항은 검색 대상에서 제외
...
def _extract_metadata(data: dict) -> dict:
    """특허 JSON에서 메타데이터 추출 (서지사항, 초록, IPC 코드)."""
...
def _group_claims(valid_claims: dict) -> dict[int, list[int]]:
    ...
    for num, claim in valid_claims.items():
        ...
        if ctype == "independent":
            ...
            # 종속항의 부모 독립항 탐색 (3단계 폴백)
            parent = None
            # 1차: 직접 참조("제N항")가 독립항인지 확인
            for r in refs:
...
def _sliding_window_dense_texts(
    ...
    while start < len(body):
        ...
        if end >= len(body):
            break
        # 이전 윈도우와 overlap_chars만큼 겹치게 이동
