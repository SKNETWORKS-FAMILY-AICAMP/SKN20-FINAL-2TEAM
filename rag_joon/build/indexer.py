...
def _get_model() -> SentenceTransformer:
    """KURE-v1 임베딩 모델 싱글톤 반환. 최초 호출 시 1회만 로드."""
    global _model
    if _model is None:
...
def _get_collection() -> chromadb.Collection:
    """ChromaDB 컬렉션 싱글톤 반환. cosine 유사도 메트릭 사용."""
...
def build_dense_index(chunks: list[dict], force: bool = False) -> int:
    ...
    # ChromaDB 메타데이터는 스칼라 타입만 허용 → 리스트는 JSON 문자열로 변환
    metadatas = []
    for c in chunks:
    ...
    while i < len(sorted_texts):
        # 적응형 배치: 긴 텍스트일수록 작은 배치로 VRAM 초과 방지
...
def build_sparse_index(chunks: list[dict], force: bool = False) -> int:
    ...
    # 각 청크의 sparse_text를 형태소 분석하여 토큰 리스트 생성
    ...
    # BM25 인덱스 빌드 (k1, b는 config에서 튜닝)
    bm25 = BM25Okapi(tokenized, k1=config.BM25_K1, b=config.BM25_B)

    # bm25 객체 + chunk_ids를 pickle로 저장 (검색 시 로드)
