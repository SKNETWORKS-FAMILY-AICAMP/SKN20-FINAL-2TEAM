"""Step 1 (Gemini): Gemini API로 테스트 데이터 추론.

기존 02_evaluate.py, 03_compare.py와 동일한 출력 형식(infer_gemini.xlsx)을 생성한다.

사용법:
    export GOOGLE_API_KEY=AIza...
    python 01_infer_gemini.py
    python 01_infer_gemini.py --model gemini-2.5-flash  # Flash 모델
    python 01_infer_gemini.py --max_samples 100          # 일부만 테스트

출력:
    output/infer_gemini25pro.xlsx  (기존 컬럼 + pred_output 컬럼)
"""
...
logger.add(
    LOG_DIR / "eval_infer_gemini_{time:YYYY-MM-DD_HH-mm-ss}.log",
    level="DEBUG",
    rotation="100 MB",
    encoding="utf-8",
)
...
def call_gemini(client, model_name: str, user_content: str, max_tokens: int, retries: int = 3) -> str:
    """Gemini API 호출. 실패 시 최대 retries회 재시도."""
...
def main():
    parser = argparse.ArgumentParser(description="Gemini 추론 (Step 1)")
    parser.add_argument("--model", default="gemini-2.5-pro-preview-05-06", help="Gemini 모델명")
    ...
    parser.add_argument("--max_tokens", type=int, default=8192, help="최대 생성 토큰 수 (thinking 포함)")
    parser.add_argument("--max_samples", type=int, default=None, help="테스트할 최대 샘플 수 (미지정 시 전체)")
    parser.add_argument("--model_name", default="gemini25pro", help="출력 파일명에 쓸 모델 이름")
    ...
    if not api_key:
        ...
        logger.error("export GOOGLE_API_KEY=AIza... 후 재실행하세요.")
        sys.exit(1)

    try:
        from google import genai
    except ImportError:
        logger.error("google-genai 패키지가 없습니다. pip install google-genai 후 재실행하세요.")
        sys.exit(1)

    client = genai.Client(api_key=api_key)

    logger.info(f"=== Gemini 추론 시작 ===")
