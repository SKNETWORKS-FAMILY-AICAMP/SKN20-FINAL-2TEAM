...
def rewrite_query(
    ...
    prompt = (
        ...
        "- 성분이나 구성요소를 빼거나 제외하라는 질문이면, 제외된 성분은 절대 언급하지 마세요.\n"
        "- 남아있는 성분/구성요소만 구체적으로 나열하여 제품 설명을 작성하세요.\n"
        "- '제외한', '빼고', '없이' 같은 부정 표현을 사용하지 마세요.\n"
        "- 성분을 추가하라는 질문이면, 기존 성분에 새 성분을 합쳐서 나열하세요.\n\n"
        "[예시 1]\n"
        "이전 대화: 사용자가 'A, B, C, D를 포함하는 화장품'에 대해 질문함\n"
        "현재 질문: B를 빼면 침해가 안될까?\n"
        "재작성: A, C, D를 포함하는 화장품\n\n"
        "[예시 2]\n"
        "이전 대화: 사용자가 'X, Y, Z를 사용한 선크림'에 대해 질문함\n"
        "현재 질문: 여기에 W를 추가하면 어떨까?\n"
        "재작성: X, Y, Z, W를 사용한 선크림\n\n"
        f"[이전 대화]\n{conv_text}\n\n"
        f"[현재 질문]\n{latest_message}\n\n"
        "[재작성된 제품 설명]\n"
    )
