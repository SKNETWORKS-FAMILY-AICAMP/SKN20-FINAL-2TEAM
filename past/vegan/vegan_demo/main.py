...
def detect_image_type(state: AgentState) -> AgentState:
    ...
    messages = [
        SystemMessage(content="이미지를 음식 사진인지, 원재료명 리스트가 적힌 글자 사진인지 분류해라. "
                              "음식 사진이면 'food',  원재료명 리스트이면 'ingredients'로 대답해라."),
        HumanMessage(content=[
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}
        ])
    ]
    response = state['client'].invoke(messages)
    state['image_type'] = response.content.lower().strip()
...
def extract_ingredients(state: AgentState) -> AgentState:
    ...
    response = state['client'].invoke(messages)
    state['extracted_ingredients'] = response.content
...
def recognize_food(state: AgentState) -> AgentState:
    ...
    messages = [
        SystemMessage(content="음식 이름과 추정 재료를 JSON으로 반환해라. 형식: {\"food_name\": \"...\", \"estimated_ingredients\": \"...\"}"),
        HumanMessage(content=[
            {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}
        ])
    ]
    client_with_json = state['client'].bind(response_format={"type": "json_object"})
    response = client_with_json.invoke(messages)
    food_info = json.loads(response.content)
...
def analyze_ingredients(state: AgentState) -> AgentState:
    ...
    try:
        messages = [
            ...
            HumanMessage(content=f"다음은 제품의 원재료명 리스트입니다. 분석해주세요: {ingredients_to_analyze}")
        ]
        client_with_json = state['client'].bind(response_format={"type": "json_object"})
        response = client_with_json.invoke(messages)
        state['analysis_result'] = json.loads(response.content)
...
class VeganAnalyzerWithLangGraph:
    ...
    def __init__(self):
        # main.py 기준으로 프로젝트 루트의 .env 파일 경로 계산
...
if __name__ == "__main__":
    ...
    # main.py 기준으로 test_image 폴더 경로 계산
    current_dir = os.path.dirname(os.path.abspath(__file__))
